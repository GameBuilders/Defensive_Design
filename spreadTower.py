"""
spreadTower.py - Slows down enemies in range
"""
...
class SpreadTower:
    ...
    def __init__(self, x, y, group, size):
        if(not SpreadTower.initialized): # Load the images
            SpreadTower.up_image = pygame.image.load(os.path.join("images", "bug_spray.png"))
            SpreadTower.up_image = pygame.transform.scale(SpreadTower.up_image, size)
            SpreadTower.down_image = pygame.image.load(os.path.join("images", "bug_spray.png"))
            SpreadTower.down_image = pygame.transform.scale(SpreadTower.down_image, size)
            SpreadTower.left_image = pygame.image.load(os.path.join("images", "bug_spray.png"))
            SpreadTower.left_image = pygame.transform.scale(SpreadTower.left_image, size)
            SpreadTower.right_image = pygame.image.load(os.path.join("images", "bug_spray.png"))
            SpreadTower.right_image = pygame.transform.scale(SpreadTower.right_image, size)
            SpreadTower.initialized = True
        self.power = DEFAULT_POWER
        self.delay = DEFAULT_DELAY
        self.sprite = pygame.sprite.Sprite()
        self.direction = DIRECTION_NONE
        self.sprite.image = SpreadTower.down_image
        self.size = size
    ...
    def update(self, time_elapsed, mapdata, enemies):
        ...
        if(self.direction == DIRECTION_NORTH):
            self.sprite.image = SpreadTower.up_image
        elif(self.direction == DIRECTION_SOUTH):
            self.sprite.image = SpreadTower.down_image
        elif(self.direction == DIRECTION_WEST):
            self.sprite.image = SpreadTower.left_image
        elif(self.direction == DIRECTION_EAST):
            self.sprite.image = SpreadTower.right_image
